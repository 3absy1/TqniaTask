...
@section('content')
    <section class="table-sec pt-3">
        <div class="container px-2 px-md-5">
            <div class="align-items-start border-bottom flex-column">
                <x-first-head label="Topups Tabel" icon="database" />

                {{-- <button class="btn-primary btn  border" type="button" data-bs-toggle="modal"
                    data-bs-target="#exampleModal">Request Topup</button>
                <x-modal buttonText="Create" modalId="exampleModal"
                    formAction="{{ route('topups.request') }}" formMethod="POST" modalTitle="Request Topup">
                ...
                <div class="container">
                    <div class="card-body">
                        ...
                        @foreach ($topupRequests as $topupRequest)
                            <x-modal modalId="editModal{{ $topupRequest->id }}"
                                formAction="{{ route('topups.approve', ['id' => $topupRequest->id]) }}"
                                formMethod="POST" modalTitle="Edit Name">

                                <label class="form-label text-1000 fs-0 ps-0 text-capitalize lh-sm mb-2" for="adminTitle">
                                    Status </label>
                                <select class="form-control" id="adminTitle{{ $topupRequest->id }}"
                                    name="status">
                                    <option value="pending" {{ $topupRequest->status == 'pending' ? 'selected' : '' }}>
                                        Pending</option>
                                    <option value="approved" {{ $topupRequest->status == 'approved' ? 'selected' : '' }}>
                                        Approved</option>
                                    <option value="rejected" {{ $topupRequest->status == 'rejected' ? 'selected' : '' }}>
                                        Rejected</option>
                                </select>
                                </x-modal>
                            {{-- <x-modal modalId="deleteModal{{ $topupRequest->id }}"
    ...
    <script>
        $(document).on('click', '.editTopupRequest', function() {
            var id = $(this).data('id');
            $.ajax({
                url: '/topups/' + id + '/edit',
                method: 'GET',
